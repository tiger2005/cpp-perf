...
3. 进入仓库根目录，跑一次 `npm install --omit=dev` 下载必须库。加上 `--omit=dev` 将会取消对 `node-addon-api` 依赖的获取，如果不加上这个开关可能会让 `npm` 指令自行启动 `node-gyp` 指令。
...
目前，我们已经编译好了 Windows 系统下监视器的 C++ Addon。源代码可以在 `addon` 文件夹中找到，其中包含三个函数：
```cpp
...
由于本人只有 Windows 系统的电脑，暂时无法编写和编译其他系统的 C++ Addon，如果你有能力的话，也欢迎你帮助我们编译其他系统的 C++ Addon。在编译后，请将其命名为 `perf_[platform]_[arch].node` 并放在 `addon` 文件夹下。在 `run` 指令中，程序将会自动引入当前系统下的监视器文件并运行。测试完成后，请开启 PR 并提供源代码和编译好的 `node` 文件。